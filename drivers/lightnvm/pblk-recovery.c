...
void pblk_close_rblk_wait(struct pblk *pblk, struct pblk_block *rblk)
{
	struct nvm_rq *rqd;
	struct bio *bio;
	DECLARE_COMPLETION_ONSTACK(wait);

	rqd = pblk_setup_close_rblk(pblk, rblk, PBLK_IOTYPE_SYNC);
	if (!rqd) {
		pr_err("pblk: could not allocate recovery request\n");
		return;
	}
