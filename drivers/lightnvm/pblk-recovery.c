...
static void pblk_free_recov_rqd(struct pblk *pblk, struct nvm_rq *rqd)
{
...
static void pblk_rec_valid_pgs(struct work_struct *work)
{
	...
	do {
		...
		if (!rqd) {
			pr_err("pblk: could not allocate recovery req.\n");
			goto fail_free_krefbuf;
		}
...
#ifdef CONFIG_NVM_DEBUG
	atomic_add(secs_to_rec, &pblk->sync_reads);
...
next:
	...
	/* TODO: We are missing the statistics - moved for 4.8 */
