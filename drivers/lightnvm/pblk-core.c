...
{
	return atomic_inc_below(&pblk->write_inflight, 200000, nr_secs);
}
				     ...
				     unsigned int nr_upd, unsigned int nr_com)
{
	...
	if (pblk_rb_space(&pblk->rwb) < nr_upd)
		goto rollback;

	if (pblk_rb_update_l2p(&pblk->rwb, nr_upd))
		goto rollback;
	...
	*pos = pblk_rb_write_pos(&pblk->rwb);
	pblk_rb_write_commit(&pblk->rwb, nr_com);

	ctx_bio = (bio->bi_rw & REQ_FLUSH) ? bio : NULL;
			       ...
			       unsigned long flags, unsigned int nr_entries)
{
	...
	ret = (ctx_bio) ? NVM_IO_OK : NVM_IO_DONE;
			    ...
			    struct pblk_lun *rlun, int is_bb, int emergency_gc)
{
	struct nvm_lun *lun = rlun->parent;
	int ret = 0;
	int flags = emergency_gc ? NVM_IOTYPE_GC : 0;

	pblk_disable_lun(rlun, lun);

	rblk = pblk_get_blk(pblk, rlun, flags);
	if (!rblk) {
 ...
 *
 */
int __pblk_submit_write(struct pblk *pblk)
{
	struct nvm_dev *dev = pblk->dev;
	struct bio *bio;
	struct nvm_rq *rqd;
...
int pblk_submit_write(void *data)
{
...
void pblk_pad_open_blks(struct pblk *pblk)
{
	...
	pblk_for_each_lun(pblk, rlun, i) {
		...
		list_for_each_entry_safe(rblk, trblk, &open_list, list) {
			...
			/* On padding I/O completes on stack. We can close the
			 * block here
			 */
			pblk_close_rblk_wait(pblk, rblk);
		}
	...
	/* Check that all open blocks have been padded */
	pblk_for_each_lun(pblk, rlun, i) {
		spin_lock(&rlun->lock_lists);
		if (!list_empty(&rlun->open_list))
			pr_err("pblk: Bad padding on LUN:%d\n",
					rlun->parent->id);
		spin_unlock(&rlun->lock_lists);
	}
}
...
try_lun:
	...
	/* If the number of free blocks in the LUN goes below the threshold,
	 * get in emergency GC mode.
	 *
	 * TODO: This should be progressive and affect the rate limiter to
	 * reduce user I/O as the disk gets more and more full. For now, we
	 * only implement emergency GC: when the disk reaches capacity, user I/O
	 * is stopped and GC is the only one adding entries to the write buffer
	 * in order to free blocks
	 */
	if (!lun_emergency_gc && lun->nr_free_blocks < pblk->gc_ths.emergency) {
	...
	if (unlikely(block_is_bad(rblk))) {
		...
		ret = pblk_replace_blk(pblk, rblk, rlun, 1, gen_emergency_gc);
		if (ret)
			goto out;
	...
	if (block_is_full(pblk, rblk)) {
		ret = pblk_replace_blk(pblk, rblk, rlun, 0, gen_emergency_gc);
		if (ret)
			goto out;
	}
