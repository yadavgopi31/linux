...
ssize_t pblk_map_set_active_luns(struct pblk *pblk, int nr_luns)
{
	...
	int *lun_blocks;
	ssize_t ret = 0;
	int old_nr_luns, cpy_luns, next_lun;
	int i;
	...
	next_lun = pblk->w_luns.next_lun;
	pblk->w_luns.next_lun = old_nr_luns = pblk->w_luns.nr_luns;
...
ssize_t pblk_map_get_luns(struct pblk *pblk, char *buf)
{
	struct pblk_lun *rlun;
	ssize_t sz = 0;
	int i;
	...
	for (i = 0; i < pblk->w_luns.nr_luns; i++) {
		rlun = pblk->w_luns.luns[i];
		sz += sprintf(buf + sz, "POS:%d, CH:%d, LUN:%d\n",
					i,
					rlun->ch,
					rlun->parent->id);
	}
