...
static int pblk_submit_io_checks(struct pblk *pblk, struct bio *bio)
{
	...
	int is_flush = (bio->bi_rw & REQ_FLUSH);
...
static int pblk_core_init(struct pblk *pblk)
{
	...
	pblk->gc_ths.emergency = pblk->nr_luns + 2;
...
static int pblk_luns_init(struct pblk *pblk, int lun_begin, int lun_end)
{
	...
	pblk->max_write_pgs = 16; /* Write 16 ppas at a time */
		       ...
		       int lun_begin, int lun_end)
{
	...
	pblk->ts_writer = kthread_create(pblk_submit_write, pblk,
								"pblk-writer");
