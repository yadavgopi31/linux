...
{
	return pblk_map_get_luns(pblk, page);
}
					...
					ssize_t len)
{
	...
	sscanf(buf, "%lu-%lu", &lba_init, &lba_end);
...
static ssize_t pblk_sysfs_l2p_map_sanity(struct pblk *pblk, const char *buf,
					 ssize_t len)
{
	struct nvm_dev *dev = pblk->dev;
	size_t c_len;
	...
	sscanf(buf, "%llx-%lu-%lu", &ppa.ppa, &lba_init, &lba_end);
	...
	if (!read_sec) {
		pr_err("Cannot allocate debug read sector\n");
		goto out;
	}
...
static ssize_t pblk_sysfs_block_meta(struct pblk *pblk, const char *buf,
				     ssize_t len)
{
	return pblk_recov_blk_meta_sysfs(pblk, buf, len);
}
#endif
