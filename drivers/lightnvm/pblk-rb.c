			...
			unsigned int size)
{
	...
	if (buf + size >= rb->data + rb->data_size) {
		/* Wrap around case */
		s1 = (unsigned int)(rb->data + rb->data_size - buf);
		s2 = size - s1;
		BUG_ON(!data);
		BUG_ON(!(data + s1));
		memcpy(buf, data, s1);
...
int pblk_rb_update_l2p(struct pblk_rb *rb, unsigned int nr_entries)
{
	...
	/* Acquire l2p pointer when updating l2p table. Protect from sync */
...
void pblk_rb_sync_l2p(struct pblk_rb *rb)
{
	...
	/* Acquire l2p pointer when updating l2p table. Protect from update */
	l2p_upd = smp_load_acquire(&rb->l2p_update);
